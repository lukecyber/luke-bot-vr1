        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '```Type``` *.update now* ```For Update luke Bot.```\n\n' + degisiklikler + '```', MessageType.text
                    ); 
                }
            }
        }
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '\n*Luke bot Working as Private! 🤹*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your luke bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using luke bot 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [luke bot] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [luke bot] --*' + 
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [luke bot] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
